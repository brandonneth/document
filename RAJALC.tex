...
%P2: HPC has a need for portability
...
\ttt{simd\_exec}, \ttt{omp\_parallel\_for\_exec}, or \ttt{cuda\_exec}.
...
%PExtra: Data optimizations are important
...
%P4: RAJA has a need for data optimizations
...
%P5: Loop chaining does data optimizations
...
%When optimizing for data locality, focus must shift from individual loops to the
...
requires no additional compilation steps.  % Excellent writing!!
...
RAJA has two built-in execution primitives: \ttt{forall} and \ttt{kernel}.
\ttt{forall} is used for unnested loops, so for this example we use
\ttt{kernel}.
We must provide three pieces of information to \ttt{kernel}. 
...
Lines 10-11 of Listing~\ref{hydroPort} show how to initialize \ttt{zrout}
as a View object.
...
% \todo{Tom and Bronis, This section should describe and cite many places
% that RAJA is being used and the kind of performance portability
% it has demonstrated.}
...
meaningful inputs.
% MMS: this is a little problematic for double blind.
%The spread of RAJA outside of LLNL and the ECP project
%remains nascent, but it has begun to appear in benchmark suites and components
%of larger systems as the ecosystem matures. 
...
%This section describes the key concepts of the loop chain abstraction.
...
%% BRONIS: Top place listings and figures to save space
%% BRONIS: That placement is cleaner typographically anyway
%
\begin{figure}[t]
...
Although RAJA does not currently provide chainable loop objects, it has many
...
%We modify the RAJA design to enable inter-loop optimizations.
...
However, in standard RAJA, computational \ttt{kernel} statements immediately execute the specified computation.
...
\subsection{Gathering Access Information}
...
%group ``close'' iterations that are likely
%to access the same data to improve data locality.
...
%inhibits traditional parallelism while overlapped tiling retains
%parallelism across tiles at the cost of some recomputation.
...
\section{Loop Transformations in RAJALC}

%With RAJALC, the final code generation step is limited by the level at which RAJA operates. 
%Instead of generating machine code from an intermediate representation, as in a compiler, RAJALC's basic unit of computation is the \verb.KernelWrapper. object.
%Furthermore, due to the restrictions of C++ templating, the number and shape of the transformation results must be the same regardless of the parameterization of the loop.
%For example, a fusion of two loops must have the same return type regardless of whether the loops have any iteration space overlap or data interaction. 
%For this reason, the results of transformations are fixed length tuples of kernels, where the length of the tuple only depends on compile-time knowledge, such as the number and dimensionality of kernels.
...
%how the information gathered from the symbolic analysis
%of data accesses can be used to determine how to implement the loop fusion and overlapping tile
%transformations while still satisfying the data dependences between loops.
...
Before we can generate the kernel for the overlapped tiling, we must
...
\label{RAJAPerfPerf}
\end{figure}
%Quartz geomean of fraction of hand-implemented variant for different thread counts: [0.9202172805886791, 0.9468095814581927, 0.9467299323639515, 0.9489040813642557, 0.9698155542522505, 0.9533653717088155]
%Lassen geomean of fraction of hand-implemented variant for different thread counts: [0.9587275593240652, 0.9887470853780274, 1.0085188158797689, 1.0236356509933864, 1.0230328686133745, 0.9513198992412492]
...
%Bassetti et al.~\cite{Bassetti98} introduce an optimization they call sliding block temporal tiling.  
%They indicate that in these relaxation algorithms such as Jacobi "[g]enerally several sweeps are 
%made".  In their experiments they use up to 8 sweeps.
%[Domain-specific library approaches to providing overlapped tiling for PDE applications.]
%The Cactus toolkit is an application framework that supports distributed memory parallelization 
%of PDE applications and includes some capability for doing overlapped tiles of computation 
%to avoid communication~\cite{Allen00cactus-gtoolkit}.


%Build to Order BLAS~\cite{belter09:_bto} provided a MATLAB-like DSL for
%specifying dense linear algebra computations that include subcomputations
%included in BLAS~\cite{Duff:1997:LTB}.
%It included a performance model and an algorithm to optimize across those
%computations.
%% BRONIS: Not clear why this is remarkable
%% and generating the resulting optimized code.
%% BRANDON: Removed this paragraph bc we don't do linear algebra computations
...
% MMS, think we should leave this out unless we are going to have some intelligent way
% to pick tile sizes.  Are we just tuning that parameter?
%[Compiler techniques that schedule within loops to do things like pick tile sizes]
%Some modeling for the selection of tile sizes for tiling within a loop nest have been developed 
%within the multicore context to tradeoff parallelism and data locality~\cite{uday08pldi,Bondhugula2016} 
%and within the GPU context for selecting hexahedral tile sizes~\cite{Prajapati2017}.
