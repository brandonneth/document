...
The transformations that these libraries surface to the programmer support a common paradigm for program optimization: identify an expensive loop or kernel, try a transformation, and repeat.
...
\todo{need to summarize that the code difference for fusion is minimal but quite large for overlapped tiling}
...
\todo{connect to the previous subsection through the fact that it also is useful to change sparse formats}
\todo{The scope of the dissertation is supporting sparse computations and schedule transformations as part of the per-loop transformation framework}
